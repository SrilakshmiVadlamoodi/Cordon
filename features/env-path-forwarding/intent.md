...
**Done:**
- [x] A binary placed in a directory outside today's fixed `roSystemDirs`
      ...
      *(`TestRun_ForwardsCallerPathDirectoryOutsideSystemDirs`)*
- [x] With the caller's `PATH` left at its own current default (nothing
      ...
      *(`TestRun_DefaultPathResolutionUnaffected`; full existing suite
      re-run clean, `go test ./...`)*
- [x] A realistic `actions/setup-node`-shaped case (a directory outside
      `/usr` added to `PATH`, holding a `node`-like binary) resolves and
      runs — the concrete scenario motivating this feature, not just a
      synthetic directory
      *(`TestRun_ForkedChildResolvesToolFromForwardedPath` — a *forked
      child* of the wrapped command resolves the forwarded tool via
      inherited `PATH`, matching how a lifecycle script invokes `node`
      by bare name, not just top-level `Command[0]` resolution)*
- [x] DECISIONS.md entry stating plainly that this widens the read-only
...
- [x] DECISIONS.md (or this doc) states explicitly *when* the caller's
      ...
      *(see "Where the read happens" below, and DECISIONS.md 2026-09-13)*

**Where the read happens, stated plainly (not left implicit):**
`sandbox.Run` (`run_linux.go`, the host-side, pre-namespace parent
process) is where `os.Getenv("PATH")` is captured (`run_linux.go:142`,
`extraPathDirs(os.Getenv("PATH"))`) — before `childConfig` is marshaled
(`run_linux.go:144`) and long before `cmd.Start()` (`run_linux.go:211`).
At that point no namespace, no re-exec'd child, no tracee, and no
wrapped-command process exists yet at all — this is strictly earlier
than the allowlist's read-before-tracee point (which happens inside the
already-re-exec'd, already-namespaced `runChild`, `run_linux.go:326`).
The captured directory list crosses the process boundary the same way
`Command`/`ProjectDir`/`NewRoot` already do today: as `childConfig`'s
`ExtraPathDirs` field, JSON-encoded into `childEnvVar` before
`cmd.Start()`. Consequence: the wrapped command cannot possibly
...
- **Missing or unreadable `PATH` directories are skipped silently**, the
  ...
  `/lib64` on arm64 (`run_linux.go:396`) — a `PATH` entry that
